...
@[hott] def is_add_hom [class] [has_add A] [has_add B] (f : A → B) : Type _ :=
∀ a₁ a₂, f (a₁ + a₂) = f a₁ + f a₂

@[hott] def respect_add [has_add A] [has_add B] (f : A → B) [H : is_add_hom f] (a₁ a₂ : A) :
  f (a₁ + a₂) = f a₁ + f a₂ := H a₁ a₂

@[hott] def is_prop_is_add_hom [instance] [has_add A] [has_add B] [is_set B] (f : A → B) :
  is_prop (is_add_hom f) :=
by unfold is_add_hom; apply _

@[hott] def is_add_hom_id (A : Type _) [has_add A] : is_add_hom (@id A) :=
take a₁ a₂, rfl

@[hott] def is_add_hom_compose [has_add A] [has_add B] [has_add C]
...
section add_group_A_B
  ...
  @[hott] def eq_zero_of_is_add_hom {f : A → B} [is_add_hom f]
      [is_embedding f] {a : A} (fa0 : f a = 0) :
    a = 0 :=
  have f a = f 0, by rewrite [fa0, respect_zero f],
  show a = 0, from is_injective_of_is_embedding this

  @[hott] theorem eq_zero_of_eq_zero_of_is_embedding {f : A → B} [is_add_hom f] [is_embedding f]
    {a : A} (h : f a = 0) : a = 0 :=
...
@[hott] def is_mul_hom [class] [has_mul A] [has_mul B] (f : A → B) : Type _ :=
∀ a₁ a₂, f (a₁ * a₂) = f a₁ * f a₂

@[hott] def respect_mul [has_mul A] [has_mul B] (f : A → B) [H : is_mul_hom f] (a₁ a₂ : A) :
  f (a₁ * a₂) = f a₁ * f a₂ := H a₁ a₂

@[hott] def is_prop_is_mul_hom [instance] [has_mul A] [has_mul B] [is_set B] (f : A → B) :
  is_prop (is_mul_hom f) :=
begin unfold is_mul_hom, apply _ end

@[hott] def is_mul_hom_id (A : Type _) [has_mul A] : is_mul_hom (@id A) :=
take a₁ a₂, rfl

@[hott] def is_mul_hom_compose [has_mul A] [has_mul B] [has_mul C]
...
section group_A_B
  ...
  @[hott] def eq_one_of_is_mul_hom {f : A → B} [is_mul_hom f]
...
@[hott] def is_ring_hom [class] {R₁ R₂ : Type _} [semiring R₁] [semiring R₂] (f : R₁ → R₂) :=
is_add_hom f × is_mul_hom f × f 1 = 1

@[hott] def is_ring_hom.mk {R₁ R₂ : Type _} [semiring R₁] [semiring R₂] (f : R₁ → R₂)
    (h₁ : is_add_hom f) (h₂ : is_mul_hom f) (h₃ : f 1 = 1) : is_ring_hom f :=
pair h₁ (pair h₂ h₃)

@[hott] def is_add_hom_of_is_ring_hom [instance] {R₁ R₂ : Type _} [semiring R₁] [semiring R₂]
    (f : R₁ → R₂) [H : is_ring_hom f] : is_add_hom f :=
prod.pr1 H

@[hott] def is_mul_hom_of_is_ring_hom [instance] {R₁ R₂ : Type _} [semiring R₁] [semiring R₂]
    (f : R₁ → R₂) [H : is_ring_hom f] : is_mul_hom f :=
prod.pr1 (prod.pr2 H)

@[hott] def is_ring_hom.respect_one {R₁ R₂ : Type _} [semiring R₁] [semiring R₂]
    (f : R₁ → R₂) [H : is_ring_hom f] : f 1 = 1 :=
prod.pr2 (prod.pr2 H)

@[hott] def is_prop_is_ring_hom [instance] {R₁ R₂ : Type _} [semiring R₁] [semiring R₂] (f : R₁ → R₂) :
...
section semiring
  variables {R₁ R₂ R₃ : Type _} [semiring R₁] [semiring R₂] [semiring R₃]
  variables (g : R₂ → R₃) (f : R₁ → R₂) [is_ring_hom g] [is_ring_hom f]

  @[hott] def is_ring_hom_id : is_ring_hom (@id R₁) :=
  is_ring_hom.mk id (λ a₁ a₂, rfl) (λ a₁ a₂, rfl) rfl

  @[hott] def is_ring_hom_comp : is_ring_hom (g ∘ f) :=
  is_ring_hom.mk _
  ...
  @[hott] def respect_mul_add_mul (a b c d : R₁) : f (a * b + c * d) = f a * f b + f c * f d :=
