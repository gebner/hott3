...
inductive int : Type _ :=
| of_nat : nat → int
| neg_succ_of_nat : nat → int

notation `ℤ` := int
@[hott] def int.of_num [coercion] [reducible] (n : num) : ℤ :=
...
@[hott] def int_has_one [instance] [priority int.prio] : has_one int :=
has_one.mk (of_nat 1)

@[hott] theorem of_nat_zero : of_nat (0:nat) = (0:int) :=
rfl

@[hott] theorem of_nat_one : of_nat (1:nat) = (1:int) :=
rfl

/- definitions of basic functions -/

@[hott] def neg_of_nat : ℕ → ℤ
| 0        := 0
| (succ m) := -[1+ m]

@[hott] def sub_nat_nat (m n : ℕ) : ℤ :=
match (n - m : nat) with
  | 0        := of_nat (m - n)  -- m ≥ n
...
@[hott] def int_has_add [instance] [priority int.prio] : has_add int := has_add.mk int.add
@[hott] def int_has_neg [instance] [priority int.prio] : has_neg int := has_neg.mk int.neg
@[hott] def int_has_mul [instance] [priority int.prio] : has_mul int := has_mul.mk int.mul

@[hott] lemma mul_of_nat_of_nat   (m n : nat) : of_nat m * of_nat n = of_nat (m * n) :=
rfl

@[hott] lemma mul_of_nat_neg_succ_of_nat (m n : nat) : of_nat m * -[1+ n] = neg_of_nat (m * succ n) :=
rfl

@[hott] lemma mul_neg_succ_of_nat_of_nat (m n : nat) : -[1+ m] * of_nat n = neg_of_nat (succ m * n) :=
rfl

@[hott] lemma mul_neg_succ_of_nat_neg_succ_of_nat (m n : nat) : -[1+ m] * -[1+ n] = succ m * succ n :=
rfl

/- some basic functions and properties -/

@[hott] theorem of_nat.inj {m n : ℕ} (H : of_nat m = of_nat n) : m = n :=
down (int.no_confusion H imp.id)

@[hott] theorem eq_of_of_nat_eq_of_nat {m n : ℕ} (H : of_nat m = of_nat n) : m = n :=
of_nat.inj H

@[hott] theorem of_nat_eq_of_nat_iff (m n : ℕ) : of_nat m = of_nat n ↔ m = n :=
iff.intro of_nat.inj !ap

@[hott] theorem neg_succ_of_nat.inj {m n : ℕ} (H : neg_succ_of_nat m = neg_succ_of_nat n) : m = n :=
down (int.no_confusion H imp.id)

@[hott] theorem neg_succ_of_nat_eq (n : ℕ) : -[1+ n] = -(n + 1) := rfl

private @[hott] def has_decidable_eq₂ : Π (a b : ℤ), decidable (a = b)
...
@[hott] def has_decidable_eq [instance] [priority int.prio] : decidable_eq ℤ := has_decidable_eq₂

@[hott] theorem of_nat_add (n m : nat) : of_nat (n + m) = of_nat n + of_nat m := rfl

@[hott] theorem of_nat_succ (n : ℕ) : of_nat (succ n) = of_nat n + 1 := rfl

@[hott] theorem of_nat_mul (n m : ℕ) : of_nat (n * m) = of_nat n * of_nat m := rfl

@[hott] theorem sub_nat_nat_of_ge {m n : ℕ} (H : m ≥ n) : sub_nat_nat m n = of_nat (m - n) :=
show sub_nat_nat m n = nat.cases_on 0 (m -[nat] n) _, from (sub_eq_zero_of_le H) ▸ rfl

section
local attribute sub_nat_nat [reducible]
@[hott] theorem sub_nat_nat_of_lt {m n : ℕ} (H : m < n) : sub_nat_nat m n = -[1+ pred (n - m)] :=
...
@[hott] def nat_abs (a : ℤ) : ℕ := int.cases_on a id succ

@[hott] theorem nat_abs_of_nat (n : ℕ) : nat_abs n = n := rfl

@[hott] theorem eq_zero_of_nat_abs_eq_zero : Π {a : ℤ}, nat_abs a = 0 → a = 0
| (of_nat m) H := ap of_nat H
| -[1+ m']   H := absurd H !succ_ne_zero

@[hott] theorem nat_abs_zero : nat_abs (0:int) = (0:nat) :=
rfl

@[hott] theorem nat_abs_one : nat_abs (1:int) = (1:nat) :=
rfl
...
@[hott] def abstr (a : ℕ × ℕ) : ℤ := sub_nat_nat (pr1 a) (pr2 a)

@[hott] theorem abstr_of_ge {p : ℕ × ℕ} (H : pr1 p ≥ pr2 p) : abstr p = of_nat (pr1 p - pr2 p) :=
sub_nat_nat_of_ge H

@[hott] theorem abstr_of_lt {p : ℕ × ℕ} (H : pr1 p < pr2 p) :
...
@[hott] theorem abstr_repr : Π (a : ℤ), abstr (repr a) = a
| (of_nat m) := (sub_nat_nat_of_ge (zero_le m))
| -[1+ m]    := rfl

@[hott] theorem repr_sub_nat_nat (m n : ℕ) : repr (sub_nat_nat m n) ≡ (m, n) :=
nat.lt_ge_by_cases
  (take H : m < n,
...
@[hott] theorem repr_abstr (p : ℕ × ℕ) : repr (abstr p) ≡ p :=
!prod.eta ▸ !repr_sub_nat_nat

@[hott] theorem abstr_eq {p q : ℕ × ℕ} (Hequiv : p ≡ q) : abstr p = abstr q :=
...
@[hott] theorem equiv_iff (p q : ℕ × ℕ) : (p ≡ q) ↔ (abstr p = abstr q) :=
iff.intro abstr_eq (assume H, equiv.trans (H ▸ equiv.symm (repr_abstr p)) (repr_abstr q))

@[hott] theorem equiv_iff3 (p q : ℕ × ℕ) : (p ≡ q) ↔ ((p ≡ p) × (q ≡ q) × (abstr p = abstr q)) :=
iff.trans !equiv_iff (iff.symm
   (iff.trans (prod_iff_right !equiv.refl) (prod_iff_right !equiv.refl)))

@[hott] theorem eq_abstr_of_equiv_repr {a : ℤ} {p : ℕ × ℕ} (Hequiv : repr a ≡ p) : a = abstr p :=
!abstr_repr⁻¹ ⬝ abstr_eq Hequiv

@[hott] theorem eq_of_repr_equiv_repr {a b : ℤ} (H : repr a ≡ repr b) : a = b :=
eq_abstr_of_equiv_repr H ⬝ !abstr_repr

section
local attribute abstr [reducible]
local attribute dist [reducible]
@[hott] theorem nat_abs_abstr : Π (p : ℕ × ℕ), nat_abs (abstr p) = dist (pr1 p) (pr2 p)
...
@[hott] theorem cases_of_nat_succ (a : ℤ) : (Σn : ℕ, a = of_nat n) ⊎ (Σn : ℕ, a = - (of_nat (succ n))) :=
int.cases_on a (take m, sum.inl (sigma.mk _ rfl)) (take m, sum.inr (sigma.mk _ rfl))

@[hott] theorem cases_of_nat (a : ℤ) : (Σn : ℕ, a = of_nat n) ⊎ (Σn : ℕ, a = - of_nat n) :=
sum.imp_right (sigma.rec (take n, (sigma.mk _))) !cases_of_nat_succ

@[hott] theorem by_cases_of_nat {P : ℤ → Type _} (a : ℤ)
...
@[hott] theorem by_cases_of_nat_succ {P : ℤ → Type _} (a : ℤ)
...
@[hott] def padd (p q : ℕ × ℕ) : ℕ × ℕ := (pr1 p + pr1 q, pr2 p + pr2 q)

@[hott] theorem repr_add : Π (a b : ℤ), repr (add a b) ≡ padd (repr a) (repr b)
...
@[hott] theorem padd_congr {p p' q q' : ℕ × ℕ} (Ha : p ≡ p') (Hb : q ≡ q') : padd p q ≡ padd p' q' :=
...
@[hott] theorem padd_comm (p q : ℕ × ℕ) : padd p q = padd q p :=
calc (pr1 p + pr1 q, pr2 p + pr2 q)
        = (pr1 q + pr1 p, pr2 p + pr2 q) : by rewrite add.comm
    ... = (pr1 q + pr1 p, pr2 q + pr2 p) : by rewrite (add.comm (pr2 p) (pr2 q))

@[hott] theorem padd_assoc (p q r : ℕ × ℕ) : padd (padd p q) r = padd p (padd q r) :=
...
@[hott] def pneg (p : ℕ × ℕ) : ℕ × ℕ := (pr2 p, pr1 p)

-- note: this is =, not just ≡
@[hott] theorem repr_neg : Π (a : ℤ), repr (- a) = pneg (repr a)
| 0        := rfl
| (succ m) := rfl
| -[1+ m]  := rfl

@[hott] theorem pneg_congr {p p' : ℕ × ℕ} (H : p ≡ p') : pneg p ≡ pneg p' := inverse H

@[hott] theorem pneg_pneg (p : ℕ × ℕ) : pneg (pneg p) = p := !prod.eta

@[hott] theorem nat_abs_neg (a : ℤ) : nat_abs (-a) = nat_abs a :=
calc
...
@[hott] theorem padd_pneg (p : ℕ × ℕ) : padd p (pneg p) ≡ (0, 0) :=
show pr1 p + pr2 p + 0 = pr2 p + pr1 p + 0,
by rewrite [nat.add_comm (pr1 p)]

@[hott] theorem padd_padd_pneg (p q : ℕ × ℕ) : padd (padd p q) (pneg q) ≡ p :=
...
@[hott] def pabs (p : ℕ × ℕ) : ℕ := dist (pr1 p) (pr2 p)

@[hott] theorem pabs_congr {p q : ℕ × ℕ} (H : p ≡ q) : pabs p = pabs q :=
calc
  pabs p = nat_abs (abstr p) : nat_abs_abstr
    ... = nat_abs (abstr q) : abstr_eq H
    ... = pabs q : nat_abs_abstr

@[hott] theorem nat_abs_eq_pabs_repr (a : ℤ) : nat_abs a = pabs (repr a) :=
calc
  nat_abs a = nat_abs (abstr (repr a)) : abstr_repr
    ... = pabs (repr a) : nat_abs_abstr

@[hott] theorem nat_abs_add_le (a b : ℤ) : nat_abs (a + b) ≤ nat_abs a + nat_abs b :=
calc
...
@[hott] theorem nat_abs_neg_of_nat (n : nat) : nat_abs (neg_of_nat n) = n :=
begin cases n, reflexivity, reflexivity end

section
local attribute nat_abs [reducible]
@[hott] theorem nat_abs_mul : Π (a b : ℤ), nat_abs (a * b) = (nat_abs a) * (nat_abs b)
...
@[hott] def pmul (p q : ℕ × ℕ) : ℕ × ℕ :=
  (pr1 p * pr1 q + pr2 p * pr2 q, pr1 p * pr2 q + pr2 p * pr1 q)

@[hott] theorem repr_neg_of_nat (m : ℕ) : repr (neg_of_nat m) = (0, m) :=
nat.cases_on m rfl (take m', rfl)

-- note: we have =, not just ≡
@[hott] theorem repr_mul : Π (a b : ℤ), repr (a * b) = pmul (repr a) (repr b)
...
@[hott] theorem equiv_mul_prep {xa ya xb yb xn yn xm ym : ℕ}
...
@[hott] theorem pmul_congr {p p' q q' : ℕ × ℕ} : p ≡ p' → q ≡ q' → pmul p q ≡ pmul p' q' := equiv_mul_prep

@[hott] theorem pmul_comm (p q : ℕ × ℕ) : pmul p q = pmul q p :=
show (_,_) = (_,_),
begin
  congruence,
...
private @[hott] theorem pmul_assoc_prep {p1 p2 q1 q2 r1 r2 : ℕ} :
...
@[hott] def int_has_sub [instance] [priority int.prio] : has_sub int :=
has_sub.mk has_sub.sub

@[hott] def int_has_dvd [instance] [priority int.prio] : has_dvd int :=
has_dvd.mk has_dvd.dvd

/- additional properties -/
@[hott] theorem of_nat_sub {m n : ℕ} (H : m ≥ n) : of_nat (m - n) = of_nat m - of_nat n :=
...
@[hott] theorem neg_succ_of_nat_eq' (m : ℕ) : -[1+ m] = -m - 1 :=
by xrewrite [neg_succ_of_nat_eq, neg_add]

@[hott] def succ (a : ℤ) := a + (succ zero)
@[hott] def pred (a : ℤ) := a - (succ zero)
@[hott] def nat_succ_eq_int_succ (n : ℕ) : nat.succ n = int.succ n := idp
@[hott] theorem pred_succ (a : ℤ) : pred (succ a) = a := !sub_add_cancel
@[hott] theorem succ_pred (a : ℤ) : succ (pred a) = a := !add_sub_cancel

@[hott] theorem neg_succ (a : ℤ) : -succ a = pred (-a) :=
by rewrite [↑succ,neg_add]

@[hott] theorem succ_neg_succ (a : ℤ) : succ (-succ a) = -a :=
by rewrite [neg_succ,succ_pred]

@[hott] theorem neg_pred (a : ℤ) : -pred a = succ (-a) :=
by rewrite [↑pred,neg_sub,sub_eq_add_neg,add.comm]

@[hott] theorem pred_neg_pred (a : ℤ) : pred (-pred a) = -a :=
by rewrite [neg_pred,pred_succ]

@[hott] theorem pred_nat_succ (n : ℕ) : pred (nat.succ n) = n := pred_succ n
@[hott] theorem neg_nat_succ (n : ℕ) : -nat.succ n = pred (-n) := !neg_succ
@[hott] theorem succ_neg_nat_succ (n : ℕ) : succ (-nat.succ n) = -n := !succ_neg_succ

@[hott] def rec_nat_on {P : ℤ → Type _} (z : ℤ) (H0 : P 0)
...
@[hott] theorem rec_nat_on_neg {P : ℤ → Type _} (n : ℕ) (H0 : P zero)
  (Hsucc : Π⦃n : ℕ⦄, P n → P (succ n)) (Hpred : Π⦃n : ℕ⦄, P (-n) → P (-nat.succ n))
